...
function find(name, props) {
  ...
  const fn = function (driver) {
    return byJSX(driver, name, filter).then((result) => {
      if (result.length === 0 && !!extractReduxComponentName(name)) {
        return reduxWrappedFind(driver, name, filter);
      }
      return result;
    });
  };
...
function reduxWrappedFind(driver, name, filter) {
  const innerName = extractReduxComponentName(name)[1];
...
function extractReduxComponentName(name) {
  return /^Wrapper\((.*)\)$/.exec(name);
}
